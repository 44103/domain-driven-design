...
describe("Book", () => {
  ...
  describe("create", () => {
    it("デフォルト値で在庫を作成する", () => {
      ...
      expect(book.bookId.isEqual(bookId)).toBeTruthy();
      expect(book.title.isEqual(title)).toBeTruthy();
      expect(book.price.isEqual(price)).toBeTruthy();
      expect(
        book.stockId.isEqual(new StockId("testIdWithExactLength"))
      ).toBeTruthy();
      expect(
        book.quantityAvailable.isEqual(new QuantityAvailable(0))
      ).toBeTruthy();
      expect(
        book.status.isEqual(new Status(StatusEnum.OutOfStock))
      ).toBeTruthy();
    });
  });
  ...
  describe("changeTitle", () => {
    it("titleを変更する", () => {
      ...
      expect(book.title.isEqual(newTitle)).toBeTruthy();
    });
  });
  ...
  describe("changePrice", () => {
    it("priceを変更する", () => {
      ...
      expect(book.price.isEqual(newPrice)).toBeTruthy();
    });
  });
});
